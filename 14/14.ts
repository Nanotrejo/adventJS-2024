function minMovesToStables(reindeer: number[], stables: number[]): number {
  const reindeerSort = reindeer.slice().sort((a, b) => a - b);
  const stablesSort = stables.slice().sort((a, b) => a - b);
  return reindeerSort.reduce((acc, curr, i) => acc + Math.abs(curr - stablesSort[i]), 0);
}
